...
hm_header('Group Training Award Course');

template('course_box');
?>
  <div class="banner">Award Which Course? &bull;</div>
  <div id="view_course_table" class="assign">
  <form action="groupawardusertocourse.php" method="post">
  <?php errOut('grouptraining');?>

  <div id="group_training_box">
   <div class="field">
    <label>Certificate Override (optional):</label> <select name="certificate_type" id="certificate_type"><?php echo optOut($certificate_types,$certificate_type,'Default Certificate');?></select>
   </div>
   <div class="field">
    <label>Custom date for certificate:</label> <input type="text" name="award_date" value="<?php echo date('Y-m-d');?>"> <em>Format YYYY-MM-DD</em>
   </div>
  </div>
   <table>
    <tr>
     <td colspan="2" style="border: 0;"><div class="controls"><button type="button" onClick="selectAll(this);">Select all</button> <button type="button" onClick="deselectAll(this);">Deselect all</button> <button type="submit">Award</button></div></td>
    </tr>
   </table>
   <table id="seriesbox">
    <tr>
...
while ($r = hm_fetch($q)) {
 ...
 $count = hm_result("SELECT count(*) FROM results".SITE_SUFFIX." r LEFT JOIN users".SITE_SUFFIX." u ON (r.userid=u.userid) WHERE r.courseid='$course_id' and r.passorfail='P' and LEFT(r.datestarted,4)=".date('Y')." and u.userlevel='10';",1);
