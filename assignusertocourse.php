...
if (count($course_ids) === 0 && $series_id === 0) {
 err('Please select a series or a course to assign.','assigncourse');
 hm_redirect('assigncourse.php');
}
...
if ($_POST['submit']) {
 ...
 if ($compliance_period > 0) {
  ...
  $comp_date = date('Y-m-d', strtotime(date('Y-m-d').$addon));
    ...
    foreach ($post_users as $user_id) {
      ...
      */
      ...
      if ($current_subseries) {
       //      $current_progress = getNextCourse($user_id,$series_id);
       sync_series($user_id,$series_id,$series_courses,$date,$comp_date);
       ...
       foreach ($series_courses as $course) {
        ...
        $insert_buffer[] = "(NULL,'$user_id','$course_id','".mysqlText($date)."','".mysqlText($comp_date)."', 'Y','$series_id','$subseries','$sort')";
//        hm_query("INSERT INTO Assignments".SITE_SUFFIX." (id,userid,course_id,dateassigned,datetakenby,active,series_id,subseries_id,series_sort) VALUES (NULL,'$user_id','$course_id','$date','$comp_date', 'Y','$series_id','$subseries','$sort')");
    ...
    // SELECT FROM Assignments WHERE userid IN ('24,23,15,18,35,36,37,38,28,11,13,3,25,30,9,31,29') and series_id='37' and (active='Y' or active='C');


    if ($total_series_users > 0) {
     err('Assigned series users to '.$total_series_users.' users, with a compliance period of '.$compliance_periods[$compliance_period].' ('.$comp_date_nice.').',get_home_slug(),1);
     go_home();
   ...
   /* Single Course Assignments */
    ...
    foreach ($course_ids as $course_id) {
     foreach ($post_users as $user_id) {
      ...
      if ($user_id > 0) {
       $safe_ids[] = $user_id;
       $result = hm_result("SELECT id FROM Assignments".SITE_SUFFIX." WHERE userid='$user_id' and course_id='$course_id' and active='Y' LIMIT 1;",1);

       if ($result < 1) {
        $users_changed[$user_id] = 1;
        $change_cnt++;
        hm_query("INSERT INTO Assignments".SITE_SUFFIX." (id,userid,course_id,dateassigned,datetakenby,active) VALUES (NULL,'$user_id','$course_id','$date','$comp_date', 'Y')");
       }
      }
     }
    }
    ...
    if ($total_courses === 1) {
     ...
     if (errCnt('assignusertocourse') === 0) {
      if ($change_cnt > 0 || $deleted_count > 0) err('Assigned '.$change_cnt.' users with a compliance period of '.$compliance_periods[$compliance_period].' ('.$comp_date_nice.') and removed '.$deleted_count.' users.',get_home_slug(),1);
      else err('No new assignments made.',get_home_slug(),2);
      go_home();
     }
    }
    ...
    if (errCnt('assignusertocourse') === 0) {
     ...
     if ($change_cnt > 0) err('Assigned '.$total_users.' users to '.$total_courses.' courses, with a compliance period of '.$compliance_periods[$compliance_period].' ('.$comp_date_nice.').',get_home_slug(),1);
     else err('No new assignments made.',get_home_slug(),2);
     go_home();
    }
...
template('course_box');
?>
  <div class="banner">Assign Course to Who? &bull;</div>
  <div id="view_course_table" class="assignto">
<?php

errOut('assignusertocourse');
$first = true;
...
if (hm_cnt($q) > 0) {
 ...
 while ($r = hm_fetch($q)) {
  ...
  if ($single_mode) {
   $check = hm_result("SELECT count(*) FROM Assignments".SITE_SUFFIX." WHERE userid='".intval($r['userid'])."' and course_id IN ($course_id_tally) and active='Y' LIMIT 1;",1);
   if ($check == $total_courses) $checked = true;
   else $checked = false;
  }

  else {
   $check = hm_result("SELECT count(*) FROM Assignments".SITE_SUFFIX." WHERE userid='".intval($r['userid'])."' and series_id='$series_id' and active != 'N';",1);
